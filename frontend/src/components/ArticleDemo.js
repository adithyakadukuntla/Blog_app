...
function ArticleDemo() {
  ...
  const saveArticle=async(editedArticle)=>{
    ...
    //make HTTP PUT
    let res=await axiosWithToken.put('http://localhost:5000/author-api/article',modifiedArticle)
  ...
  const deleteArticleById=async()=>{
    ...
    delete copy._id;
    let res=await axiosWithToken.put(`http://localhost:5000/author-api/article/${editedArticle.articleId}`,copy)
        ...
        <div>
          {currentUser.userType === "author" && (
            <>
              ...
              <button className="me-2 btn btn-danger" onClick={restoreArticleById}>
